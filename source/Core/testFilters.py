...
def plot2d(filtros, norm = False, makeoutput = None):
	...
	if norm: z_p = z_p/np.max(z_p)
	
	z_s = np.array([ filtro.snrs for filtro in filtros ])
	if norm: z_s = z_s/np.max(z_s)
...
if __name__ == '__main__':
	...
	if not args.preview and not args.graph and not args.trisurf and not args.mesh and not args.justpreview:
		print("Nothing to do!")
		sys.exit(1)
	...
	if args.preview or args.justpreview:
		...
		trace_copy.detrend('linear').filter('bandpass', corners = 4, freqmin = 5.0, freqmax = 15.0)
		t = np.arange(trace_copy.stats.npts) * trace_copy.stats.delta
		
		a, b = noisewindow.t - trace_copy.stats.starttime - noisewindow.w / 2, noisewindow.t - trace_copy.stats.starttime + noisewindow.w / 2
		plt.axvspan(a, b, alpha = 0.5, label = 'Noise Window', color = 'red')
		
		a, b = pwindow.t - trace_copy.stats.starttime - pwindow.w / 2, pwindow.t - trace_copy.stats.starttime + pwindow.w / 2
		plt.axvspan(a, b, alpha = 0.5, label = 'P-Window', color = 'yellow')
		
		a, b = swindow.t - trace_copy.stats.starttime - swindow.w / 2, swindow.t - trace_copy.stats.starttime + swindow.w / 2
		...
		if args.makeoutput:
			plt.savefig("{}.{}.{}.{}_{}_preview.png".format(N,S,L,C,pwindow.t))
		else:
			plt.show()
		
		plt.close()
		if args.justpreview: sys.exit(0)
	
	filtros = filterCombos(1., 35., 2., 12.)
	ratios(trace, filtros, noisewindow,
	                       pwindow,
	                       swindow)
	
	if args.graph or args.trisurf:
		plot3d(filtros, args.trisurf, makeoutput = "{}.{}.{}.{}_{}".format(N,S,L,C,pwindow.t) if args.makeoutput else None)
	
	if args.mesh:
		plot2d(filtros, makeoutput = "{}.{}.{}.{}_{}".format(N,S,L,C,pwindow.t) if args.makeoutput else None)
